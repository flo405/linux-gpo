...
func writeManagedTag(tagsDir, key, value string) (string, error) {
	primary := filepath.Join(tagsDir, key+".tag")
	content := "# managed-by: lgpod-inventory\n" + strings.TrimSpace(value) + "\n"

	target := primary
	if st, err := os.Stat(primary); err == nil && !st.IsDir() {
		// see if it's ours
		existing, _ := os.ReadFile(primary)
		if !strings.HasPrefix(string(existing), "# managed-by: lgpod-inventory") {
			// avoid overwriting admin-managed tag
			target = filepath.Join(tagsDir, "inventory."+key+".tag")
		}
	}
...
func SyncInventoryTags(cacheDir, tagsDir, deviceKeyPath string) (string, int, error) {
	...
	if err != nil {
		// Not fatal for policy runs—return hash so caller can log it and continue.
        // (The agent can still run with locally-present tags.)
		return hash, 0, err
	}
	...
	if match == nil {
		// No mapping for this device—nothing to write.
		return hash, 0, nil
	}

	// Write each tag as a file
	wrote := 0
	for k, v := range match.Tags {
		...
		if _, err := writeManagedTag(tagsDir, k, v); err != nil {
			// continue writing other tags but return a combined error at the end
			return hash, wrote, fmt.Errorf("write tag %q: %w", k, err)
		}
		wrote++
	}

	// Optionally write identity tag (useful for debugging/policy audit)
	if match.Identity != "" {
