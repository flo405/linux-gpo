...
func Render(p *Policy) ([]byte, string, error) {
	...
	for _, bad := range []string{"eval(", "Function(", "require(", "import("} {
		if strings.Contains(js, bad) {
			return nil, "", fmt.Errorf("forbidden token in output")
		}
	}
...
func matchCond(m Match) string {
	if m.ActionID != "" {
		return "action.id === " + jsString(m.ActionID)
	}
...
func subjectCond(s Subject) string {
	...
	if s.Active != nil {
		if *s.Active {
			parts = append(parts, "isActive()")
		} else {
			parts = append(parts, "!isActive()")
		}
	}
	if s.Group != "" {
		parts = append(parts, "inGroup("+jsString(s.Group)+")")
	}
	if s.User != "" {
		parts = append(parts, "isUser("+jsString(s.User)+")")
	}
	if len(parts) == 0 {
		return ""
	}
